...
export const useOrganizationData = (
  ...
  const [organizationsRefreshTrigger, setOrganizationsRefreshTrigger] =
    useState(0);
  ...
  const convertToTableFormat = useCallback(
    (org: DatabaseOrganization): TableOrganization => ({
      id: org.id.toString(),
      name: org.name,
        ...
        try {
          if (conditionalAPI && "getDashboardOverview" in conditionalAPI) {
            ...
            if (conditionalApiWithOverview.getDashboardOverview) {
              response =
                await conditionalApiWithOverview.getDashboardOverview();
            } else {
    ...
    async (page = 1, search = "", append = false) => {
      try {
        ...
        try {
          if (
            conditionalAPI &&
            "getOrganizationsList" in conditionalAPI &&
            conditionalAPI.getOrganizationsList
          ) {
            response = await conditionalAPI.getOrganizationsList(
              page,
              5,
              search,
            );
