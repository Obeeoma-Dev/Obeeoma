...
export interface ConditionalAPI {
  ...
  getOrganizationsClientDistribution?: () => Promise<
    APIResponse<ClientDistributionResponse>
  >;
  [key: string]: unknown;
}
  ...
  const fetchOrganizations = useCallback(
    async (pageNum = 1, search = "") => {
      try {
        ...
        setError(null);
        console.log(
          `Context: Fetching organizations: page=${pageNum}, search="${search}"`,
        );
        ...
        console.log(
          `Context: Processed ${formattedOrgs.length} organizations, total: ${totalCount}, hasMore: ${hasNext}`,
        );
      } catch (error) {
