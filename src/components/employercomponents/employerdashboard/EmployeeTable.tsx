...
const EmployeeTable = ({ employees }: { employees: Employee[] }) => {
  ...
  const [viewMode, setViewMode] = useState<"table" | "view" | "update">("table");
  ...
  const handleSave = async () => {
    ...
    try {
      await updateEmployeeValidationSchema.validate(formData, { abortEarly: false });
      setValidationErrors({});
  ...
  const handleToggleStatus = async (id: number, currentStatus: string) => {
    try {
      await dispatch(toggleEmployeeStatus({ id: id.toString(), currentStatus })).unwrap();
      setModalMessage("Status updated successfully");
      setModalType("success");
    } catch {
      setModalMessage("Failed to update status");
  ...
  const columns = [
    ...
    columnHelper.accessor("employeedepartment", { 
        header: "Department",
        cell: (info) => info.getValue() || <span className="text-muted italic">Not Assigned</span>
    }),
    columnHelper.accessor("status", {
      header: "Status",
      cell: (info) => (
        <span
          className="badge text-white"
          style={{ backgroundColor: info.getValue() === "active" ? THEME_COLOR : "#b4beb9dc" }}
        >
          {(info.getValue() || "N/A").toUpperCase()}
        </span>
      ),
      ...
      cell: ({ row }) => (
        <div className="d-flex justify-content-end align-items-center gap-2">
          <button
            className="btn btn-sm"
            style={{
              color: row.original.status === "inactive" ? THEME_COLOR : "#6c757d",
              borderColor: row.original.status === "inactive" ? THEME_COLOR : "#6c757d",
            }}
          ...
          <Dropdown align="end">
            <Dropdown.Toggle as="button" className="btn btn-sm p-1 border-0 no-caret bg-transparent">
              <i className="bi bi-three-dots-vertical fs-5 text-dark"></i>
            </Dropdown.Toggle>
            <Dropdown.Menu className="shadow border-0">
              <Dropdown.Item onClick={() => { setSelectedEmployee(row.original); setViewMode("view"); }}>View</Dropdown.Item>
              <Dropdown.Item onClick={() => { setSelectedEmployee(row.original); setViewMode("update"); }}>Update</Dropdown.Item>
              <Dropdown.Item className="text-secondary" onClick={() => handleDelete(row.original.id)}>Delete</Dropdown.Item>
            </Dropdown.Menu>
          </Dropdown>
        </div>
      ),
    ...
    return (
      <div className="p-4">
        ...
        <div className="card p-3 shadow-sm border-0">
          ...
          <p><strong>Department:</strong> {selectedEmployee.employeedepartment || "N/A"}</p>
  ...
  if (viewMode === "update" && selectedEmployee) {
    return (
      <div className="p-4" style={{ maxWidth: "900px" }}>
        <button className="btn btn-link p-0 mb-4 text-decoration-none d-flex align-items-center gap-2" 
                style={{ color: THEME_COLOR }} onClick={handleBackToTable}>
          <i className="bi bi-arrow-left"></i> Back to Employee Table
        </button>
          ...
          <div className="card-body p-4">
            ...
            <div className="mb-3 w-100 text-start">
              ...
              <input
                ...
                onChange={handleInputChange}
              />
              <div className="invalid-feedback">{validationErrors.emailAddress}</div>
            </div>
            ...
            <div className="mb-3 w-100 text-start">
              ...
              <input
                ...
                onChange={handleInputChange}
              />
              <div className="invalid-feedback">{validationErrors.employeedepartment}</div>
            </div>
            ...
            <Button
              ...
              disabled={isActionLoading}
            >
              {isActionLoading ? <><Spinner size="sm" className="me-2" /> Updating...</> : "Save Changes"}
            </Button>
          </div>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <div className="d-flex justify-content-between align-items-center mt-3 pb-4 px-3">
        <div className="text-muted">Showing {table.getFilteredRowModel().rows.length} of {employees.length} employees</div>
        <div className="d-flex gap-2">
          <button className="btn btn-sm btn-outline-secondary" onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()}>Previous</button>
          <button className="btn btn-sm btn-outline-secondary" onClick={() => table.nextPage()} disabled={!table.getCanNextPage()}>Next</button>
        </div>
      </div>
