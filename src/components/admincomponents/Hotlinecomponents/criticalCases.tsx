...
const styles = {
  ...
  header: { display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "1.5rem" },
  ...
  return (
    <Card style={styles.card} className="mb-4">
      ...
      <div style={styles.header}>
        ...
        <h5 style={{ margin: 0, fontWeight: 600, fontFamily: 'heading' }}>Critical Cases</h5>
      </div>
      ...
      <div>
        {cases.map((c) => (
          <div key={c.id} style={styles.caseContainer(c.type)}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", fontFamily: 'body' }}>
              <div>
                {/* Case title */}
                ...
                <div style={styles.metaWrapper}>
                  ...
                  <span style={{ ...styles.metaText, margin: "0 0.25rem" }}>•</span>
                  <span style={styles.metaText}>Assigned to: {c.assignedTo}</span>
                </div>
              </div>

              {/* Status badge */}
              <Badge bg={c.type === "critical" ? "danger" : "warning"}>{c.status}</Badge>
            </div>
          </div>
        ))}
      </div>
