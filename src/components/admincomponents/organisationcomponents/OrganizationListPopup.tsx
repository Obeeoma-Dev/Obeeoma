...
const OrganizationListPopup: React.FC<OrganizationListPopupProps> = ({ show, onHide }) => {
    ...
    const fetchOrganizations = async (pageNum = 1, search = "") => {
        try {
            ...
            const response = await adminAPI.getOrganizationsList(pageNum, 10, search);
            ...
            console.log(`Processed: ${results.length} organizations, total: ${totalCount}, hasMore: ${hasMore}`);
    ...
    const filteredOrganizations = organizations.filter(org =>
        ...
        (org.location?.toLowerCase().includes(searchTerm.toLowerCase()) || false)
    );
                        ...
                        <tbody>
                            {filteredOrganizations.length === 0 && !loading ? (
                                ...
                                filteredOrganizations.map((org, index) => (
                                    <tr
                                        key={org.id}
                                        ref={index === filteredOrganizations.length - 1 ? lastOrganizationElementRef : null}
                                    >
                                        ...
                                        <td>
                                            <span className={`badge ${org.current_plan === 'Premium' ? 'bg-success' : 'bg-secondary'}`}>
                                                {org.current_plan}
                                            </span>
                                        </td>
                                        <td>
                                            <span className={`badge ${org.is_active ? 'bg-success' : 'bg-danger'}`}>
