...
import { updateEmployeeLocal, deleteEmployeeLocal, updateEmployee, deleteEmployee, toggleEmployeeStatus } from "../../store/slices/EmployerSlice";
...
const [bulkConfirmAction, setBulkConfirmAction] = useState<'activate_all' | 'deactivate_all' | 'delete_all' | null>(null);
...
const handleEditFieldChange = (field: string, value: string | number | boolean) => {
...
console.log('Dispatching updateEmployeeLocal with:', { id: editModalId, ...editFormData });
...
console.log('Calling updateEmployee API with payload:', { id: editModalId, ...editFormData });
const result = await dispatch(updateEmployee({ id: editModalId, ...editFormData }));
...
console.log('confirmStatusChange dispatching', { id: statusChangeId, status: statusChangeAction });
...
// call API toggle
const result = dispatch(toggleEmployeeStatus({ id: String(statusChangeId), currentStatus: statusChangeAction === 'activate' ? 'inactive' : 'active' }));
...
dispatch(updateEmployeeLocal({ id: emp.id, status: 'active' }));
dispatch(toggleEmployeeStatus({ id: String(emp.id), currentStatus: emp.status === 'active' ? 'inactive' : 'active' }));
});
} else if (bulkConfirmAction === 'deactivate_all') {
console.log('Deactivating all employees');
filteredEmployees.forEach((emp) => {
dispatch(updateEmployeeLocal({ id: emp.id, status: 'inactive' }));
dispatch(toggleEmployeeStatus({ id: String(emp.id), currentStatus: emp.status === 'active' ? 'inactive' : 'active' }));
});
} else if (bulkConfirmAction === 'delete_all') {
console.log('Deleting all employees');
filteredEmployees.forEach((emp) => {
dispatch(deleteEmployeeLocal(emp.id));
dispatch(deleteEmployee(String(emp.id)));
});
}
...
VIEW MODAL
======================== */}
<Modal show={viewModalId !== null} onHide={closeViewModal} centered size="lg">
...
<p className="text-muted">{getEmployeeById(viewModalId)?.emailAddress}</p>
</div>
<div className="mb-3">
<label className="fw-semibold">Department:</label>
<p className="text-muted">{getEmployeeById(viewModalId)?.employeedepartment}</p>
</div>
<div className="mb-3">
<label className="fw-semibold">Status:</label>
<p>
<Badge bg={getEmployeeById(viewModalId)?.status === 'active' ? 'success' : 'secondary'}>
...
EDIT MODAL
======================== */}
<Modal show={editModalId !== null} onHide={closeEditModal} centered size="lg">
<Modal.Header closeButton>
...
onChange={(e) => handleEditFieldChange('employeedepartment', e.target.value)}
/>
</Form.Group>
...
======================== */}
<Modal show={statusChangeId !== null} onHide={closeStatusChangeModal} centered>
<Modal.Header closeButton>
<Modal.Title>
{statusChangeAction === 'activate' ? 'Activate Employee' : 'Deactivate Employee'}
</Modal.Title>
</Modal.Header>
<Modal.Body>
{statusChangeAction === 'activate' ? (
<p>Are you sure you want to <strong>Activate</strong> this employee?</p>
) : (
<p>Are you sure you want to <strong>Deactivate</strong> this employee?</p>
)}
</Modal.Body>
<Modal.Footer>
...
<Button
variant={statusChangeAction === 'activate' ? 'success' : 'warning'}
...
<Modal.Body>
<p>Are you sure you want to <strong>delete</strong> this employee? This action cannot be undone.</p>
</Modal.Body>
<Modal.Footer>
...
======================== */}
<Modal show={bulkConfirmAction !== null} onHide={closeBulkConfirmModal} centered>
<Modal.Header closeButton>
<Modal.Title>
