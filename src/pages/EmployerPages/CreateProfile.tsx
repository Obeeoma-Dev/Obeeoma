...
const schema = z.object({
  website: z
    .string()
    .refine((v) => v === "" || urlRegex.test(v), "Website must be a valid URL"),
...
const CreateProfile: React.FC = () => {
  ...
  const [errors, setErrors] = useState<
    Partial<Record<keyof OrganizationProfile, string>>
  >({});
  ...
  function handleChange<K extends keyof OrganizationProfile>(
    key: K,
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setErrors((prev) => ({
        ...prev,
        website: err?.message || "Submission failed",
      }));
    } finally {
      setSubmitting(false);
    }
      ...
      <form onSubmit={handleSubmit} noValidate>
        <div style={{ display: "flex", gap: 12 }}>
          <label style={{ flex: 1 }}>
            ...
            />
            {errors.website && (
              <div style={{ color: "crimson" }}>{errors.website}</div>
            )}
          </label>
        ...
        <label style={{ display: "block", marginTop: 12 }}>
          ...
          />
          {errors.linkedin && (
            <div style={{ color: "crimson" }}>{errors.linkedin}</div>
          )}
        </label>
