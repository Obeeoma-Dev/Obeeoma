...
import CallLogTable, { type CallLog } from "../../../components/admincomponents/Hotlinecomponents/callLogTable";
import CriticalCases, { type Case } from "../../../components/admincomponents/Hotlinecomponents/criticalCases";
import OperatorPerformance, { type Operator } from "../../../components/admincomponents/Hotlinecomponents/operatorPerformance";
...
interface HotlineApiResponse {
  ...
  operator_performance?: Array<{ name?: string; calls?: number; resolution_rate?: number }>;
}
...
const HotlineActivity: React.FC = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) setError(e instanceof Error ? e.message : "Failed to load hotline data");
      } finally {
  ...
  const criticalCasesList: Case[] = (data?.critical_cases ?? []).map((c) => ({
    ...
    type: (c.urgency === "critical" ? "critical" : "warning") as "critical" | "warning",
  }));

  const operatorList: Operator[] = (data?.operator_performance ?? []).map((o) => ({
    name: o.name ?? "—",
    ...
    return (
      <SystemAdminLayout title="Hotline Activity">
        <Container fluid className="py-4">
          ...
          <Row>
            <Col md={6}><HourlyCallChart /></Col>
            <Col md={6}><CallReasonsChart /></Col>
          </Row>
          <CallLogTable />
          <Row className="g-4">
            <Col xs={12} lg={6}><CriticalCases /></Col>
            <Col xs={12} lg={6}><OperatorPerformance /></Col>
          </Row>
        </Container>
      </SystemAdminLayout>
    );
  ...
  return (
    <SystemAdminLayout title="Hotline Activity">
      <Container fluid>
        ...
        <Row className="g-4">
          <Col xs={12} lg={6}>
            <CriticalCases cases={criticalCasesList.length ? criticalCasesList : undefined} />
          </Col>
          <Col xs={12} lg={6}>
            <OperatorPerformance operators={operatorList.length ? operatorList : undefined} />
          </Col>
        </Row>
      </Container>
    </SystemAdminLayout>
