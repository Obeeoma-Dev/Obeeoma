...
jest.mock("../store/slices/authSlice", () => {
  ...
  mockConfirmMfa.fulfilled = {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    match: (action: any) =>
      action && action.type === "auth/confirmMfa/fulfilled",
  };
  mockConfirmMfa.rejected = {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    match: (action: any) =>
      action && action.type === "auth/confirmMfa/rejected",
  };
  return {
    ...actual,
    __esModule: true,
...
describe("MfaSetupPage Component", () => {
  ...
  test("calls setupMfa on mount when no setup data exists", () => {
    renderWithProviders(<MfaSetupPage />, {
      preloadedState: {
        auth: { mfaSetupData: null, isMfaSetupConfirmed: false },
      },
    });
  ...
  test("shows success message and close button when MFA is confirmed", () => {
    ...
    expect(
      screen.getByText(/Multi-Factor Authentication is Active/i),
    ).toBeInTheDocument();
  ...
  test("displays warning when mfaSetupData is null and not loading", () => {
    ...
    expect(
      screen.getByText(/Could not load MFA setup data/i),
    ).toBeInTheDocument();
  });
});
