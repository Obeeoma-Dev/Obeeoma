  ...
  async (payload: PaymentUpdatePayload, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        axiosError.response?.data || "Failed to update payment method.",
      );
    }
  },
);
...
export const fetchBillingHistory = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      const axiosError = error as any;
      return rejectWithValue(
        axiosError.response?.data?.detail || "Failed to fetch billing history.",
      );
    }
  },
);
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(
        ...
        (state, action: PayloadAction<InvoiceItem[]>) => {
          state.status = "succeeded";
      ...
      .addCase(fetchBillingHistory.rejected, (state, action) => {
        state.status = "failed";
        state.error =
          (action.payload as string) || "Unknown error fetching history.";
      });
  },
});
